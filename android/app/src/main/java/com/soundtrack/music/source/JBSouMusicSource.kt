...
/**
 * JBSou JBSouMusicClient 移植：https://www.jbsou.cn/
 * - 搜索：POST https://www.jbsou.cn/  (form: input, filter=name, type=<source>, page)
 ...
 * - 歌词：lrc 拼 base 后 GET（取 </script> 之后内容）
 * 该源无夸克依赖，可直接拿到外链。原 Python 需先拉首页维持 session，本端未持久 cookie，可能部分失败。
 */
class JBSouMusicSource : MusicSource {
    override val id = "jbsou"
    override val label = "JBSou"

    private val BASE = "https://www.jbsou.cn/"
    ...
    private val searchHeaders = mapOf(
        ...
        "Origin" to "https://www.jbsou.cn",
        "X-Requested-With" to "XMLHttpRequest",
        "Referer" to "https://www.jbsou.cn/",
                ...
                try {
                    ...
                    for (i in 0 until data.length()) {
                        ...
                        list.add(
                            Song(
                                songId = songId,
                                source = this@JBSouMusicSource.id,
                                title = title,
    ...
    override suspend fun fetchLyric(song: Song): String? = withContext(Dispatchers.IO) {
        ...
        try {
            val req = Request.Builder().url(lrcUrl)
                ...
                .get().build()
            val resp = Net.client().newCall(req).execute()
            val text = resp.body?.string().orEmpty().substringAfter("</script>")
